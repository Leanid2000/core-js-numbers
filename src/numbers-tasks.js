...
function toNumber(value, def) {
  const a = !Number.isNaN(Number(value));
  const b = Number(value);
  return typeof Number(value) === 'number' && a ? b : def;
}
...
function getFibonacciNumber(index) {
  ...
  return index === 0 ? 0 : arr[arr.length - 1];
}
...
function getSumOfNumbers(x1, x2, x3) {
  const a = (x1 + x2 + x3).toFixed(10);
  const b = Math.max(x1, x2, x3);
  const c = b.toString().length;
  return a[0] === '0' ? Number(a).toFixed(c - 2) : Number(a);
}
